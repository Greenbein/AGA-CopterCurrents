...
% Flatten pixels and keep only valid indices before calling sub2ind.
% This avoids "Out of range subscript" when projected points are outside image.
pixels_2D_X_v = pixels_2D_X(:);
pixels_2D_Y_v = pixels_2D_Y(:);
ind2use = isfinite(pixels_2D_X_v) & isfinite(pixels_2D_Y_v);
pixels_2D_X_v = pixels_2D_X_v(ind2use);
pixels_2D_Y_v = pixels_2D_Y_v(ind2use);

% get linear index corresponding to the original image
LinearInd_img = sub2ind(size(img),pixels_2D_Y_v,pixels_2D_X_v);
...
LinearInd_grid_all = sub2ind(size(grid_X_RW_2d_pix),grid_Y_RW_2d_pix(:),grid_X_RW_2d_pix(:));
LinearInd_grid = LinearInd_grid_all(ind2use);
