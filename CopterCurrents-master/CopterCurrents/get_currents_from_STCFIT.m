...
% ------------- New ------------------
if isfield(SG_fit, 'is_interpolated')
    is_interpolated = [SG_fit(:).is_interpolated];
else
    is_interpolated = false(size(Ux));
end
% ------------------------------------
...
Camera_currents = struct('x',x,'y',y,'Ux',Ux,'Uy',Uy,'SNR',SNR,...
    ...
    'currentdir_flag',currentdir_flag,'is_interpolated', is_interpolated);
