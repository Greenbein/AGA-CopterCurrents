...
% ============ Green and blue QUIVER ============
if isfield(currents, 'is_interpolated')
    idx_interp = currents.is_interpolated == true;
else
    idx_interp = false(size(currents.Ux));
end

idx_real = ~idx_interp;

% Blue arrows for fit algotithm
if any(idx_real)
    quiver(currents.x(idx_real), currents.y(idx_real), ...
           currents.Ux(idx_real) * arrow_scale, currents.Uy(idx_real) * arrow_scale,...
           'b', 'Linewidth', 3, 'AutoScale', 'off');
end

% Green arrow for interpolation
if any(idx_interp)
    quiver(currents.x(idx_interp), currents.y(idx_interp), ...
           currents.Ux(idx_interp) * arrow_scale, currents.Uy(idx_interp) * arrow_scale,...
